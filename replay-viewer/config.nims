...
# use-after-free fixed in cc_replay.nim (emscripten_exit_with_live_runtime)
# was found. Keep this so any future stale free crashes at the fault instead
# of corrupting replay playback at a distance.
--define:useMalloc

# client/art is preloaded because the RENDERER reads it at runtime: global.nim
# opens client/art/walls/wall_h.jpg and client/art/lockerroom/bg.jpg while
# baking the track bed and the horizon, and under emscripten that path has to
# exist in MEMFS or the bake raises inside the worker with no other symptom
# than a board that never draws. It is the one link-flag change beyond the
# renames ctf's file needed.
#
...
# cc_error_len(). Aborting keeps linear memory intact, and the page reads
# cc_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "cc_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_cc_load_replay,_cc_frame,_cc_input,_cc_packet_ptr,_cc_packet_len,_cc_mismatch_tick,_cc_error_ptr,_cc_error_len,_cc_stage_ptr,_cc_stage_len
  """).replace("\n", " ")
)
